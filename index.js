...
module.exports = async (req, res) => {
  ...
  // Function to identify output index to be manipulated
  ...
  //Toggled Survey Options
  // var optionIndex = output.SurveyElements.getIndexBy('PrimaryAttribute', 'Survey Options')
  // var toggleSurveyOptions = {
  //   QuesPerPage:function(){
  //     if (input.hasOwnProperty("SurveyOptions")&& input.SurveyOptions.hasOwnProperty("QuestionsPerPage")){
  //     output.SurveyElements[optionIndex].Payload.QuestionsPerPage = input.SurveyOptions.QuestionsPerPage
  //     }
  //   }
  // }

  // toggleSurveyOptions.QuesPerPage()
  var item = {
    'SurveyID': ' ',
    'Element': 'SQ',
    'PrimaryAttribute': ' ',
...
displayQuestionNum()
  ...
  //   for (let times = 1; times <= dup[question]; times++) {
  //     array.push({QuestionID: 'QID' + question})
  //   }
  //   dupArray[ref + ''] = array.concat()
  //   ref++
  // }
...
// console.log(JSON.stringify(output.SurveyElements[output.SurveyElements.length-2],null,2))

// fs.writeFileSync('./output.qsf', JSON.stringify(output))

// // fs.writeFileSync('./output.qsf', JSON.stringify(output).replace(/\//g, String.raw`\/`))

  if (output.error.length>0){
    send(res, 200, output.error)
  }else{
    send(res, 200, output)
  }
}